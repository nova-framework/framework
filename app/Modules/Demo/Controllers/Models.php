...
class Models extends BaseController
{
    ...
    public function index()
    {
        ...
        $message .= '<h4>$this->model->limit(2, 0)->findAll();</h4><pre>'.var_export($members, true).'</pre><br>';
        ...
        $message .= '<h4>$userInfo</h4><pre>'.var_export($userInfo, true).'</pre><br>';

        $retval = $this->model->insert($userInfo);

        $message .= '<h4>$this->model->insert($userInfo);</h4><pre>'.var_export($retval, true).'</pre><br>';

        //
        $members2 = $this->model->findAll();

        $message .= '<h4>$this->model->findAll()</h4><pre>'.var_export($members2, true).'</pre><br>';

        //
        $retval = $this->model->deleteBy('username', 'virgil');

        $message .= '<h4>$this->model->deleteBy(\'username\', \'virgil\');</h4><pre>'.var_export($retval, true).'</pre><br>';

        //
        $members3 = $this->model->findAll();

        $message .= '<h4>$this->model->findAll()</h4><pre>'.var_export($members3, true).'</pre><br>';

        //
        $members4 = $this->model->orderBy('username', 'desc')->limit(2, 0)->findAll();

        $message .= '<h4>$this->model->orderBy(\'username\', \'desc\')->limit(2, 0)->findAll();</h4><pre>'.var_export($members4, true).'</pre><br>';

        //
        $result = $this->model->findBy('username', 'marcus');

        $message .= '<h4>$this->model->findBy(\'username\', \'marcus\');</h4><pre>'.var_export($result, true).'</pre><br>';
        ...
        $message .= '<h4>$this->model->orderBy(\'username\', \'desc\')->findMany(array(1, 3));</h4><pre>'.var_export($members5, true).'</pre><br>';
