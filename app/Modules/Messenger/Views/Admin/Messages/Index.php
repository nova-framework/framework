    ...
    <?php foreach ($threads->getItems() as $thread) { ?>
        ...
        <div class="callout callout-<?= $class; ?>" style="margin: 0 0 10px 0; padding: 10px;">
            ...
            <p class="last-child <?= ($class == 'default') ? 'text-muted' : ''; ?>">
                <small><?=  __d('messenger', '<strong>Creator:</strong> {0}', $thread->creator()->username); ?></small> |
                <small><?=  __d('messenger', '<strong>Participants:</strong> {0}', $thread->participantsString($userId)); ?></small>
            </p>
        </div>
    <?php } ?>
    </div>
