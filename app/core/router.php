...
class Router {
	...
	// Enabled by default in index.php, although, you can dectivate it by: Router::$fallback = false;
	public static $fallback;

	// Halts - Don't process other routes when match is found
	public static $halts = true;

	// Routes, Methods, callbacks, error callback
	...
	public static function invokeObject($callback, $matched = null, $msg = null) {
		...
		// grab all parts based and collect lat index of the array
	...
	public static function autoDispatch() {
		...
		// set controller, method and args
		$controller = $uri !== ''	   && isset($parts[0])  ? $parts[0] : DEFAULT_CONTROLLER;
		$methods    = $uri !== ''	   && isset($parts[1])  ? $parts[1] : DEFAULT_METHOD;
