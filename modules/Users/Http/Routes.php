...
Route::group(array('prefix' => '', 'namespace' => 'Modules\Users\Http\Controllers'), function()
{
    ...
    Route::post('login',  array('middleware' => 'guest|csrf', 'uses' => 'Authorize@postLogin'));
    ...
    Route::post('password/remind', array('middleware' => 'guest|csrf', 'uses' => 'Authorize@postRemind'));

    // The Password Reset.
    Route::get( 'password/reset/{token}', array('middleware' => 'guest',      'uses' => 'Authorize@reset'));
    Route::post('password/reset',         array('middleware' => 'guest|csrf', 'uses' => 'Authorize@postReset'));

    // The Account Registration.
    Route::get( 'register',                 array('middleware' => 'guest',      'uses' => 'Registrar@create'));
    Route::post('register',                 array('middleware' => 'guest|csrf', 'uses' => 'Registrar@store'));
...
Route::group(array('prefix' => 'admin', 'namespace' => 'Modules\Users\Http\Controllers\Admin'), function()
{
    ...
    Route::post('profile', array('middleware' => 'auth|csrf', 'uses' => 'Profile@update'));
    ...
    Route::post('users',              array('middleware' => 'auth|csrf', 'uses' => 'Users@store'));
    Route::get( 'users/{id}',         array('middleware' => 'auth',      'uses' => 'Users@show'));
    Route::get( 'users/{id}/edit',    array('middleware' => 'auth',      'uses' => 'Users@edit'));
    Route::post('users/{id}',         array('middleware' => 'auth|csrf', 'uses' => 'Users@update'));
    Route::post('users/{id}/destroy', array('middleware' => 'auth|csrf', 'uses' => 'Users@destroy'));
});
