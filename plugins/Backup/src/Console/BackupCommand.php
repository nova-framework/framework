...
class BackupCommand extends BaseCommand
{
    ...
    public function handle()
    {
        ...
        if ($status === true) {
            ...
            if (! empty($fileName)) {
                $this->info(__d('backup', 'Database backup was successful. Saved to {0}', $this->filePath));
            } else {
                $this->info(__d('backup', 'Database backup was successful. {0} was saved in the dumps folder.', $this->fileName));
            }
        } else {
            $this->error(__d('backup', 'Database backup failed. {0}', $status));
        }
    }
