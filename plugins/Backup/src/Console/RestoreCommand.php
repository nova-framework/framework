...
class RestoreCommand extends BaseCommand
{
	...
	protected function restoreDump($fileName)
	{
		...
		if ($status === true) {
			$this->info(__d('backup', '{0} was successfully restored.', $fileName));
		} else {
			$this->error(__d('backup', 'Database restore failed.'));
		}
	}
	...
	protected function listAllDumps()
	{
		...
		if ($finder->count() > 0) {
			$this->info(__d('backup', 'Please select one of the following dumps:') ."\n");
			...
			$this->info(__d('backup', 'You haven\'t saved any dumps.'));
		}
	}
	...
	protected function uncompress($fileName)
	{
		...
		if ($this->console->run($command) !== true) {
			$this->error(__d('backup', 'Uncompress of gzipped file failed.'));
		}
