...
namespace AcmeCorp\Forensics;

use Nova\Support\Facades\Config;
use Nova\Support\Facades\DB;
use Nova\Support\Facades\Request;

use AcmeCorp\Forensics\Console;
use AcmeCorp\Forensics\PdoDebugger;
...
class Profiler
{
    ...
    {
        $config = Config::get('forensics::config');
    ...
    public function gatherFrameworkData()
    {
        ...
        // GET variables
        if (count($_GET) == 0) {
            $output['get'] = __d('forensics', 'No GET data exists');
        } else {
            $output['get'] = array();
        ...
        // POST variables
        if (count($_POST) == 0) {
            $output['post'] = __d('forensics', 'No POST data exists');
        } else {
            $output['post'] = array();
    ...
    {
        Console::log(__d('forensics', 'Forensics - Profiler start gathering the information'));
        ...
        Console::logSpeed(__d('forensics', 'Forensics - Profiler start displaying the information'));
