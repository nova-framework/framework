...
class Builder extends BaseBuilder
{
	...
	public function getNodeData($id, $required = false)
	{
		...
		$data = $this->query->first([ $this->model->getLeftName(), $this->model->getRightName() ]);
	...
	public function whereAncestorOf($id)
	{
		...
		if ($id instanceof Node) {
			...
			$this->query->addBinding($id->getLeft());
	...
	{
		$this->query->whereBetween($this->model->getLeftName(), $values, $boolean, $not);
	...
	protected function wrappedColumns()
	{
		...
		return array(
			$grammar->wrap($this->model->getLeftName()),
			$grammar->wrap($this->model->getRightName()),
		);
	}
	...
	public function defaultOrder($dir = 'asc')
	{
		...
		$this->query->orderBy($this->model->getLeftName(), $dir);
	...
	public function moveNode($key, $position)
	{
		...
		$query = $this->query->where(function (QueryBuilder $inner) use ($boundary)
		{
			$inner->whereBetween($this->model->getLeftName(), $boundary);

			$inner->orWhereBetween($this->model->getRightName(), $boundary);
		});
	...
	public function makeGap($cut, $height)
	{
		...
		$this->query->where(function (QueryBuilder $inner) use ($cut)
		{
			$inner->where($this->model->getLeftName(), '>=', $cut);

			$inner->orWhere($this->model->getRightName(), '>=', $cut);
		});
	...
	protected function patch(array $params)
	{
		...
		foreach (array($this->model->getLeftName(), $this->model->getRightName()) as $col) {
	...
	public function countErrors()
	{
		...
		foreach ($checks as $key => $check) {
			$expression = new Expression('(select count(1) '.$check.') as '.$key);
