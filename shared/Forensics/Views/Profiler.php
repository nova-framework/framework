...
<div id="pQp" class="console">
    <table id="pqp-metrics" cellspacing="0">
        <tr>
            <td class="green" onclick="changeTab('console');">
                <var><?= $logCount; ?></var>
                <h4><?= __d('shared', 'Console'); ?></h4>
            </td>
            <td class="blue" onclick="changeTab('speed');">
                <var><?= $speedTotal; ?></var>
                <h4><?= __d('shared', 'Load Time'); ?></h4>
            </td>
            <td class="purple" onclick="changeTab('queries');">
                <var><?= __d('shared', '{0} Queries', $queryCount); ?></var>
                <h4><?= __d('shared', 'Database'); ?></h4>
            </td>
            <td class="orange" onclick="changeTab('memory');">
                <var><?= $memoryUsed; ?></var>
                <h4><?= __d('shared', 'Memory Used'); ?></h4>
            </td>
            <td class="red" onclick="changeTab('files');">
                <var><?= __d('shared', '{0} Files', $fileCount); ?></var>
                <h4><?= __d('shared', 'Included'); ?></h4>
            </td>
            <td class="white" onclick="changeTab('variables');">
                <var><?= __d('shared', 'Variables'); ?></var>
                <h4><?= __d('shared', 'Server Headers'); ?></h4>
            </td>
        </tr>
    </table>
    ...
    <div id='pqp-console' class='pqp-box'>
        <?php if ($logCount == 0) { ?>
            <h3><?= __d('shared', 'This panel has no log items.'); ?></h3>
        <?php } else { ?>
            <table class='side' cellspacing='0'>
            <tr>
                <td class='alt1'><var><?= $output['logs']['logCount']; ?></var> <h4><?= __d('shared', 'Logs'); ?></h4></td>
                <td class='alt2'><var><?= $output['logs']['errorCount']; ?></var> <h4><?= __d('shared', 'Errors'); ?></h4></td>
            </tr>
            <tr>
                <td class='alt3'><var><?= $output['logs']['memoryCount']; ?></var> <h4><?= __d('shared', 'Memory'); ?></h4></td>
                <td class='alt4'><var><?= $output['logs']['speedCount']; ?></var> <h4><?= __d('shared', 'Speed'); ?></h4></td>
            </tr>
            </table>
    ...
    <div id="pqp-speed" class="pqp-box">
        ...
        <?php } else { ?>
            <table class='side' cellspacing='0'>
                <tr><td><var><?= $output['speedTotals']['total']; ?></var><h4><?= __d('shared', 'Load Time'); ?></h4></td></tr>
                <tr><td class='alt'><var><?= $output['speedTotals']['allowed']; ?> s</var> <h4><?= __d('shared', 'Max Execution Time'); ?></h4></td></tr>
            </table>
    ...
    <div id='pqp-queries' class='pqp-box'>
        ...
        <?php } else { ?>
            <table class='side' cellspacing='0'>
            <tr><td><var><?= $output['queryTotals']['count'] ?></var><h4><?= __d('shared', 'Total Queries'); ?></h4></td></tr>
            <tr><td class='alt'><var><?= $output['queryTotals']['time'] ?></var> <h4><?= __d('shared', 'Total Time'); ?></h4></td></tr>
            <tr><td class='last'><var>0</var> <h4><?= __d('shared', 'Duplicates'); ?></h4></td></tr>
            </table>
                ...
                <?php foreach($output['queries'] as $query) { ?>
                        <tr>
                            <td class="<?= $class; ?>">
                                ...
                                <em>
                                    <?= __d('shared', 'Possible keys: <b>{0}</b>', isset($query['explain']['possible_keys']) ? $query['explain']['possible_keys'] : ''); ?> &middot;
                                    <?= __d('shared', 'Key Used: <b>{0}</b>', isset($query['explain']['key']) ? $query['explain']['key'] : ''); ?> &middot;
                                    <?= __d('shared', 'Type: <b>{0}</b>', isset($query['explain']['type']) ? $query['explain']['type'] : ''); ?> &middot;
                                    <?= __d('shared', 'Rows: <b>{0}</b>', isset($query['explain']['rows']) ? $query['explain']['rows'] : ''); ?> &middot;
                                    <?= __d('shared', 'Speed: <b>{0}</b>', $query['time']); ?>
                                </em>
                                <?php } ?>
                            </td>
                        </tr>
    ...
    <div id="pqp-memory" class="pqp-box">
        ...
        <?php } else { ?>
            <table class='side' cellspacing='0'>
                <tr><td><var><?= $output['memoryTotals']['used']; ?></var><h4><?= __d('shared', 'Memory Used'); ?></h4></td></tr>
                <tr><td class='alt'><var><?= $output['memoryTotals']['total']; ?></var> <h4><?= __d('shared', 'Total Available'); ?></h4></td></tr>
            </table>
    ...
    <div id='pqp-files' class='pqp-box'>
            <table class='side' cellspacing='0'>
                <tr><td><var><?= $output['fileTotals']['count']; ?></var><h4><?= __d('shared', 'Total Files'); ?></h4></td></tr>
                <tr><td class='alt'><var><?= $output['fileTotals']['size']; ?></var> <h4><?= __d('shared', 'Total Size'); ?></h4></td></tr>
                <tr><td class='last'><var><?= $output['fileTotals']['largest']; ?></var> <h4><?= __d('shared', 'Largest'); ?></h4></td></tr>
            </table>
            <table class='main' cellspacing='0'>
                <?php $class = ''; ?>
    ...
    <div id='pqp-variables' class='pqp-box'>
        ...
        <?php } else { ?>
            ...
            <?php foreach(array('get', 'post', 'headers') as $section) { ?>
                <?php
                    if ($section == 'get') {
                        $title = __d('shared', 'GET Variables');
                    } else if($section == 'post') {
                        $title = __d('shared', 'POST Variables');
                    } else if($section == 'headers') {
                        $title = __d('shared', 'Server Headers');
                    }
                ?>
    ...
    <table id="pqp-footer" cellspacing="0">
        <tr>
            ...
            <td class="actions">
                <a href="#" onclick="hideProfiler(); return false"><?= __d('shared', 'Hide'); ?></a>
                <a href="#" onclick="toggleDetails(); return false"><?= __d('shared', 'Details'); ?></a>
                <a class="heightToggle" href="#" onclick="toggleHeight(); return false"><?= __d('shared', 'Height'); ?></a>
            </td>
        </tr>
    </table>
</div>
